...
export default function LoveCalculator() {
  ...
  <div className="hidden md:block absolute top-1/3 -right-24 h-56 md:h-72 w-56 md:w-72 rounded-full bg-terra/20 dark:bg-pink-400/20 blur-3xl" />
      {showConfetti && (
        <div className="pointer-events-none">
          <Confetti recycle={false} numberOfPieces={viewport.w < 640 ? 120 : viewport.w < 1024 ? 220 : 300} width={viewport.w} height={viewport.h} />
        </div>
      )}

      {/* Floating emojis for fun */}
    <motion.div
  className="hidden sm:block absolute text-sage dark:text-pink-400 text-3xl sm:text-4xl"
    ...
    <motion.div
  className="hidden sm:block absolute text-terra dark:text-rose-400 text-4xl sm:text-5xl"
    ...
    <motion.div
  className="hidden sm:block absolute text-sage/70 dark:text-pink-300/70 text-2xl sm:text-3xl"
      ...
      <Card className="w-full max-w-md sm:max-w-lg lg:max-w-xl mx-auto shadow-2xl rounded-3xl dark:bg-white/5">
        <CardContent className="p-5 sm:p-6 md:p-8 text-center space-y-4 sm:space-y-6 dark:text-white" ref={cardRef}>
          <motion.h1
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1 }}
            className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-sage to-terra drop-shadow dark:from-pink-400 dark:to-rose-500"
          >
            💞 Love Calculator 💞
          </motion.h1>
          <p className="text-charcoal/70 dark:text-white/80 italic text-sm sm:text-base">
          ...
          <Button
            ...
            className="w-full text-base sm:text-lg md:text-xl py-4 sm:py-5 bg-gradient-to-r from-sage via-beige to-terra text-charcoal hover:opacity-95 dark:from-pink-500 dark:via-fuchsia-500 dark:to-rose-500 dark:text-white"
          >
            Calculate ❤️
          </Button>
          ...
          {result !== null && !loading && (
            <div className="pt-2">
              <Button
                ...
                className="w-full text-sm sm:text-base py-3 sm:py-3.5 mt-2 bg-gradient-to-r from-emerald-400 via-sage to-terra text-charcoal hover:opacity-95 dark:from-pink-600 dark:via-fuchsia-600 dark:to-rose-600 dark:text-white"
          ...
          {result !== null && !loading && (
            ...
            >
              <h2 className="text-5xl sm:text-6xl md:text-7xl font-extrabold text-sage dark:text-pink-300 drop-shadow-md">
                {result}%
              </h2>
              <p className="text-base sm:text-lg mt-3 text-charcoal/80 dark:text-white/85 font-semibold">
                {getMessage(result)}
              </p>
            </motion.div>
          )}
